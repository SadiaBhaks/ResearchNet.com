...
export default function SummarizerPage() {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Connection to Gemini failed.");
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className={`p-6 rounded-[2.5rem] border flex flex-col transition-all ${darkMode ? 'bg-slate-900/40 border-white/10' : 'bg-slate-200/50 border-slate-300'}`}>
           <div className="flex items-center justify-between mb-4 px-2">
             ...
             {summary && (
               <button onClick={() => { navigator.clipboard.writeText(summary); toast.success("Copied!"); }} className="p-2.5 hover:bg-sky-500/10 rounded-xl text-sky-500 transition-colors">
                 <Copy size={20} />
               </button>
             )}
           </div>
